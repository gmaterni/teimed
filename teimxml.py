...
class Med2Xml(object):
    ...
    def get_args_end(self, line_num, line, start, max_len):
        ...
        try:
            ...
            self.logerr.log("ERROR get_args_end().  Not found ')' ")
            self.logerr.log("%s)%s" % (line_num, line.strip()))
            return None
        return pe
    ...
    def get_tag_ent(self, line_num, line, index):
        ...
        try:
            ...
            self.logerr.log("ERROR  get_tag_ent()  ';'' Not Found. ")
            self.logerr.log(f"line num:{line_num} line:{line}")
            return None
    ...
    def build_tag_val_args(self, tag_txt, tag_data):
        ...
        if len(args) != narg:
            self.logerr.log("Warning num.args !")
            self.logerr.log(f"{self.LINE_NUM}) {self.LINE_TEXT.strip()}")
            self.logerr.log(f"{l_num} num.args:{l_text}")
            self.logerr.log("")
        for arg in args:
            if arg != '':
    ...
    def log_error(self, err, line_num, line, tag_name):
        ...
        self.logerr.log("%s  tag:%s Not Found." % (err, tag_name))
        self.logerr.log("%s) %s" % (line_num, line))
        # sys.exit(1)
        ...
        try:
            ...
            with open(self.path_src, "rt") as f:
                for line in f:
                    ...
                    self.log.log("")
                    self.log.log(f"{line_num}) {line.strip()}")
                    try:
                        s = self.compose(0, line, line_num)
                    except Exception as e:
                        self.logerr.log("ERROR compose ")
                        self.logerr.log(str(e))
                        self.logerr.log(f"{line_num}) {line}")
                        continue
                    self.log.log("=>  %s" % (s.strip()))
                    # chiamata per argomenti funzioni
                    if self.find_chs_set(s):
                        self.log.log("%s] %s" % (line_num, s.strip()))
    ...
    def check_txt(self):
        ...
        with open(self.path_out, "rt") as f:
            for i, line in enumerate(f):
                if line.find('&') > -1:
                    self.logerr.log("ERROR tag & ")
                    r = src[i].strip()
                    s = f"{i+1}) {r}"
                    self.logerr.log(s)
                    self.logerr.log(line)
                    self.logerr.log("")
