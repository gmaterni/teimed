...
from teimprjtextmake import do_main as do_main_prj_text
...
class TeimXmlFh(object):
    ...
    def __init__(self, prj_name=None):
        ...
        self.lbl_prj_var = None
        self.lbl_witness_var = None
        self.lbl_text_var = None

    # nome del prj selezionato
    def set_prj(self):
        self.prj_name = TPM.prj_name
    ...
    def add_menu_text(self):
        wt_names = TPM.witness_text_names
        names = []
        for wt in wt_names:
            w, t = wt
    ...
    def add_menu_prj_witness(self):
        lst = TPM.cmds[self.witness_name]
        rows = []
        for wid in W_IDS:
    ...
    def add_menu_prj_text(self, text_name):
        ...
        lst = TPM.txt_cmds[wtn_name][text_name]
        rows = []
        for tid in T_IDS:
    ...
    def make_prj_text(self):
        ...
        if witness_name is None:
            mbox.showinfo("Warning", "witness Undefined")
            return
        path = TPM.prj_dir
        os.chdir(path)
        do_main_prj_text(prj_name, witness_name)
        os.chdir(self.pwd)
    ...
    def add_bottom_bar(self):
        ...
        def new_label(str_var, col):
            lbl = tk.Label(bottom_bar,
                           ...
                           bg=BG_BOTTOM,
                           font=FONT_LABEL)
            lbl.grid(row=0, column=col,sticky='ws',padx=10)
            return lbl

        bottom_bar = tk.Frame(self.root)
        bottom_bar.config(height=25, bg=BG_BOTTOM, bd=0)
        bottom_bar.grid(row=1, column=0, sticky='swe', padx=0, pady=0)
    ...
    def open_root(self):
        ...
        mv_list.add_command(label='Witness Files', command=self.file_witness_list)
